...
import {
  ...
  Upload, FileText, Video, Image, Link2, X, Users, Search, Loader2, ShieldCheck, ShieldOff, Ban, UserCheck, Mail, Send, CheckCheck, CircleSlash
} from 'lucide-react';
...
export default function AdminPage() {
  ...
  // Limpiar selección cuando cambia el filtro o se sale del tab Contenido
  useEffect(() => {
  ...
  const handleBulkDelete = async () => {
    ...
    await Promise.all(ids.map(async (id) => {
      try {
        ...
        console.error('Failed to delete publication', id, err);
        failCount++;
      }
    }));
    ...
    if (failCount === 0) {
      showToast(`${successCount} publicación${successCount === 1 ? '' : 'es'} eliminada${successCount === 1 ? '' : 's'}`, 'success');
    } else if (successCount === 0) {
      showToast('No se pudo eliminar ninguna publicación', 'error');
    } else {
      showToast(`${successCount} eliminadas, ${failCount} fallaron`, 'info');
    }
  };
  ...
  return (
    <div className="min-h-screen bg-slate-100 dark:bg-slate-900 p-3 sm:p-6 md:p-8 overflow-x-hidden">
            ...
            >
                ...
                <div className="flex items-center gap-3 flex-wrap">
                  {publications.length > 0 && (
                    <button
                      ...
                      className={`inline-flex items-center gap-1.5 px-3 py-1.5 rounded-full text-xs font-semibold transition-all border ${selectedIds.size === publications.length
                        ? 'bg-usm-blue text-white border-usm-blue hover:bg-blue-700'
                        : 'bg-blue-50 dark:bg-blue-900/30 text-usm-blue dark:text-blue-300 border-blue-200/60 dark:border-blue-800 hover:bg-blue-100 dark:hover:bg-blue-900/50'
                        }`}
                    >
                      {selectedIds.size === publications.length
                        ? <><CircleSlash className="w-3.5 h-3.5" /> Deseleccionar todas</>
                        : <><CheckCheck className="w-3.5 h-3.5" /> Seleccionar todas</>}
                    </button>
                  )}
              ...
              {selectedIds.size > 0 && (
                <motion.div
                  ...
                  animate={{ opacity: 1, y: 0 }}
                  className="flex flex-wrap items-center justify-between gap-3 mb-4 p-4 rounded-2xl bg-gradient-to-r from-blue-50 to-indigo-50 dark:from-blue-900/25 dark:to-indigo-900/15 border border-blue-200/60 dark:border-blue-800/40 shadow-sm"
                >
                  <div className="flex items-center gap-2.5">
                    <span className="w-8 h-8 rounded-full bg-usm-blue text-white flex items-center justify-center shadow-md shadow-blue-500/30 shrink-0">
                      <CheckCheck className="w-4 h-4" />
                    </span>
                    <p className="text-sm font-semibold text-usm-blue dark:text-blue-200">
                  ...
                  <div className="flex items-center gap-2">
                    <button
                      ...
                      className="px-3 py-2 rounded-xl text-xs font-semibold text-slate-600 dark:text-slate-300 bg-white/70 dark:bg-slate-800/50 border border-slate-200/60 dark:border-slate-700 hover:bg-white dark:hover:bg-slate-700 transition-colors disabled:opacity-50"
                    >
                      Cancelar
                    </button>
                    <button
                      ...
                      disabled={isBulkDeleting}
                      className="inline-flex items-center gap-1.5 px-3.5 py-2 rounded-xl text-xs font-semibold bg-red-500 text-white shadow-md shadow-red-500/25 hover:bg-red-600 transition-colors disabled:opacity-50"
                    >
              ...
              <div className="space-y-3 max-h-[540px] overflow-auto pr-1">
                ...
                {publications.map((item) => {
                  ...
                  return (
                    <article
                      key={item.id}
                      className={`border rounded-xl p-4 flex flex-col md:flex-row md:items-center gap-3 md:gap-5 transition-all ${isSelected
                        ? 'border-usm-blue bg-blue-50/60 dark:bg-blue-900/15 dark:border-blue-500 shadow-sm'
                    ...
                    >
                      <div
                        ...
                        aria-label={`Seleccionar publicación: ${item.title}`}
                        className="flex items-start gap-3 flex-1 min-w-0 cursor-pointer select-none focus:outline-none focus-visible:ring-2 focus-visible:ring-usm-blue/40 rounded-lg -m-1 p-1"
                      >
                        <span
                          className={`w-6 h-6 rounded-full flex items-center justify-center shrink-0 mt-0.5 border-2 transition-all ${isSelected
                            ? 'bg-usm-blue border-usm-blue text-white shadow-md shadow-blue-500/30 scale-105'
                            : 'bg-white dark:bg-slate-700 border-slate-300 dark:border-slate-500 group-hover:border-usm-blue'
                            }`}
                        >
      ...
      <ConfirmDialog
        ...
        title={`Eliminar ${selectedIds.size} publicación${selectedIds.size === 1 ? '' : 'es'}`}
        message={`¿Estás seguro de eliminar ${selectedIds.size} publicación${selectedIds.size === 1 ? '' : 'es'}? Esta acción no se puede deshacer.`}
      />
